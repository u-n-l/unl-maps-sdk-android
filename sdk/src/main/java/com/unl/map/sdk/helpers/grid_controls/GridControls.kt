...
/**
 * [loadGrids] is an Extension method which helps us to get Grid Lines from
 * [UnlCore] Lib.
 *
 * To get the lines we need current [CellPrecision] and Maps current
 * visible [Bounds].
 *
 * @param isVisibleGrids isVisibleGrids is used to check whether used has
 *     clicked Grid Button on Map or not.
 * @param unlMapView UnlMapView is used for instance of [UnlMapView].
 * @param cellPrecision CellPrecision is used to get GridLines from
 *     [UnlCore] Lib.
 */
...
fun MapboxMap?.loadGrids(
     ...
     * Here we add 4 more Map bounds to current visible bounds as per height
     * and width of the Map.
     */
    val bounds = Bounds(
        latLngBounds.latNorth + mapBoundsWidth,
    ...
    this.getStyle { style ->
        ...
        /**
         * Here we have used a conditional constraint to check whether ZoomLevel of
         * my Map is Greater or equal to Minimum Zoom to show Grid
         *
         * And [isVisibleGrids] is used whether a user clicked on Grid Button or
         * not.
         */
        if (zoomLevel >= minZoom!! && isVisibleGrids) {
            ...
            /**
             * [UnlCore.gridLines] is an static method in [UnlCore] Lib. and returns
             * the all possible line coordinates shown to user according to selected
             * [CellPrecision] and Map [Bounds].
             */
            val lines = UnlCore.gridLines(bounds, getCellPrecisions()[cellPrecision]!!)
            /**
             * [GlobalScope.executeAsyncTask] helps us to convert GridLines to
             * [GeoJsonSource] in background Task.
             */
            GlobalScope.executeAsyncTask(onPreExecute = {
                ...
                /**
                 * lines.distinctBy method is used to iterate latLngs of lines And will be
                 * formatted in [GeoJsonSource] format.
                 */
                lines.distinctBy {
            ...
            /**
             * This condition used to check if there is any layers on Map then set the
             * visibility of the Layers None.
             */
...
/**
 * [drawLines] is an Extension method for [MapboxMap] and is used to draw
 * the Grid Lines on [UnlMapView].
 *
 * @param featureCollection featureCollection contains data related to
 *     GridLines.
...
fun MapboxMap?.drawLines(featureCollection: FeatureCollection, context: Context) {
    this?.getStyle { style ->
        /**
         * Here the purpose of this condition is to check whether list of [Feature]
         * is null or not.
         */
        if (featureCollection.features() != null) {
            ...
            if (featureCollection.features()!!.isNotEmpty()) {
                ...
                /**
                 * Here the purpose of this condition is to check whether a Grid is already
                 * created or not if created then update the data only if not then we will
                 * create in else statement.
                 */
                if (src != null) {
                    ...
                    /**
                     * Creating a Layer and Binding Layer Id and Source to [UnlMapView] style
                     * with properties like *line size* and *line color*.
                     */
                    style.addLayer(
                        LineLayer(
                            LayerIDs.GRID_LAYER_ID.name,
                            SourceIDs.GRID_SOURCE_ID.name
                        )
                            .withProperties(
...
/**
 * [setGridControls] is an Extension method for [UnlMapView] to control
 * Grid or Grid button.
 *
 * @param context context is used for inflate GridControls on [MapView]
 * @param showGrid showGrid is a boolean param and is used to
 *     enable/disable GridControls on [UnlMapView].
 */
fun UnlMapView.setGridControls(
    ...
    /**
     * Here the purpose of this condition is to check whether a user need the
    ...
    if (showGrid) {
        ...
        /**
         * Here the purpose of this static Int values is to set the margins for our
...
/**
 * [getCell] method is to create [GridCell] Object from [UnlCore.encode]
 * method and that Object contains locationId and siz of the cell
 *
...
/**
 * [locationIdToLngLat] method is used get [LatLng] from [UnlCore] Lib.
 * according to selected location Id.
 *
...
/**
 * [locationIdToBoundsCoordinates] method is used to get list of
 * points/boundaries of selected Cell from Grid
 *
 * on the basis of [locationId] from [UnlCore] Lib. using [UnlCore.bounds]
 * method.
 *
...
/**
 * {CoroutineScope.executeAsyncTask} method is used to create a background
 * Task using [CoroutineScope].
 *
 * @param onPreExecute
